...
function CouponTab(tabId,pageId,total,pageSize){
	...
	this.activeIndex = 0;
	this.prevIndex=null;
	this.pageInfo = [{pageIndex:1,totalPage:totalPage,status:0,pageSize:pageSize},
...
CouponTab.prototype.getCoupons=function(){
	...
	this.locked=true;
	$.ajax({
		url:baseRoot+'/coupon/findByPage.html',
		type:'post',
		dataType : "json",
		data:{
		...
		success:function(result){
			...
			if(pageInfo.pageIndex===1){
				pageInfo.totalPage = Math.ceil(result.totalCount/pageSize);
			}
			//loading.hide();
			if(result.totalCount>0){
				it.createCouponHtml(result);
			}else{
				it.showEmpty();
			}
		}
...
CouponTab.prototype.createCouponHtml=function(data){
	...
	for(var i=0,n=data.result.length;i<n;i++){
		oFragment.appendChild(createHtml(data.result[i]));
	}
