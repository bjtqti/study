...
var filterData = {
	...
	getSiderData:function (){
		...
		$.ajax({
			url:baseRoot+'/category/findScreenData.h',
			dataType : "json",
...
Pages.prototype.getPageData=function(pageIndex){
	...
	this.lock = true;
	$.ajax({
		url:baseRoot+'/product/findProductListByPage.h',
		dataType : "json",
...
Pages.prototype.createPageHtml=function(data){
	...
	var temp = '<a href="'+baseRoot+'/sp-'+data.code+'.h">'+
                '<div>'+
                    '<div class="pic">'+
                        '<img class="lazy" data-original="'+(data.product.picListWap[0].url)+'" />'+
...
function lazyLoad(){
	$("img.lazy").lazyload({
		placeholder : baseRoot+"/resources/images/load.gif",
		effect: "fadeIn"
	});
}
